...
object WearablePaths {
    const val CMD_START_STREAM = "/scentravn/cmd/start"
    const val CMD_STOP_STREAM = "/scentravn/cmd/stop"
    const val CMD_PING = "/scentravn/cmd/ping"

    const val EVT_HEART_RATE = "/scentravn/evt/hr"
    const val EVT_ACCEL = "/scentravn/evt/accel"
    const val EVT_STEPS = "/scentravn/evt/steps"
    const val EVT_BATTERY = "/scentravn/evt/battery"
    const val EVT_STATE = "/scentravn/evt/state"

    const val DATA_SESSION = "/scentravn/data/session"

    /** Capability declared by the watch app so phone can find it via NodeClient. */
    const val CAP_WATCH_APP = "scentravn_watch_app"
}
