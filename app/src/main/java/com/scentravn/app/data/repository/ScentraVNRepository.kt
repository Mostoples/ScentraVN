package com.scentravn.app.data.repository
...
import com.scentravn.app.data.db.SampleEntity
import com.scentravn.app.data.db.SampleType
import com.scentravn.app.data.db.SessionDao
import com.scentravn.app.data.db.SessionEntity
import com.scentravn.app.signal.EegPipeline
 ...
 */
@Singleton
class ScentraVNRepository @Inject constructor(
    ...
    private suspend fun processForDerivedSignals(sample: BioSample) {
        when (sample) {
            ...
            // Stress = HR-based arousal index (rolling-mean HR) mapped through the
            // user's 3-point calibration against the watch's own stress reading.
            // See [StressCalibrator]. Absolute HRV from averaged BPM isn't valid,
            // but a per-user calibrated mapping tracks the watch number well.
            is BioSample.HeartRate -> {
                stressCalibrator.feedHr(sample.bpm)
                stressCalibrator.calibratedStress()?.let { _stressScore.value = it }
            }
            else -> Unit
        }
    }
